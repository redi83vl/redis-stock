 ...
 */
public class Invoice extends ArrayList<Invoice.Entry> {
	...
	public Invoice(String code, Instant instant, Employee author, Client target, boolean paid) {
		this.code = code;
	...
	public static boolean add(Invoice invoice){
		...
		try(Connection cn = SQL.getConnection()) {
			...
			ps.setBoolean(5, invoice.isPaid());
			ps.execute();
